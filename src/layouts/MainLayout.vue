<template>
  <div class="min-h-screen">
      ...
      >
        <div v-if="isMobileMenuOpen" class="fixed inset-0 z-50 bg-white flex flex-col">
          ...
          <div class="px-4 py-4 border-b border-border flex items-center justify-between">
            ...
            <button @click="isMobileMenuOpen = false" class="p-2 rounded-md hover:bg-accent transition-colors">
          ...
          <div class="px-4 py-4 border-t border-border">
            <button @click="logout" class="w-full text-left px-2 py-3 text-base text-destructive flex items-center gap-3 rounded-md hover:bg-destructive/10 transition-colors">
    ...
    <!-- Desktop Sidebar -->
    <aside v-if="!isMobile" :class="['bg-sidebar border-r border-border fixed top-0 left-0 h-screen z-50 transition-all duration-300 ease-in-out', isSidebarCollapsed ? 'w-16' : 'w-64']">
      <div class="p-4 border-b border-border flex items-center justify-between">
        ...
        <button @click="toggleSidebar" class="p-1.5 rounded-md hover:bg-accent transition-colors flex-shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" :class="['transition-transform duration-300', isSidebarCollapsed ? 'rotate-180' : '']"><path d="m15 18-6-6 6-6"/></svg>
        </button>
      </div>
      <nav class="mt-4 space-y-1 px-2">
        ...
        <router-link to="ledger" custom v-slot="{ navigate, isActive }">
          <Button @click="navigate" :variant="isActive ? 'secondary' : 'ghost'" class="w-full justify-start">
            <span class="mr-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/><path d="M8 7h6"/><path d="M8 11h8"/></svg></span>
      ...
      <div :class="['absolute bottom-0 p-4 border-t border-border', isSidebarCollapsed ? 'w-16' : 'w-64']">
        <Button @click="logout" variant="ghost" class="w-full justify-start text-destructive hover:text-destructive-foreground hover:bg-destructive">
          <span class="mr-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg></span>
      ...
      <header class="bg-background shadow-sm z-10 border-b border-border">
        <div class="px-4 md:px-6 py-4 flex justify-between items-center">
          <div class="flex items-center gap-3">
            <button v-if="isMobile" @click="isMobileMenuOpen = true" class="p-1.5 rounded-md hover:bg-accent transition-colors">
            ...
            <DropdownMenu>
              <template #trigger>
                <Button variant="ghost" class="flex items-center space-x-2">
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="hidden md:block text-foreground"><path d="m6 9 6 6 6-6"/></svg>
                </Button>
              </template>
              <template #content>
...
const handleMobileNav = (navigate: () => void) => {
